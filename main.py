...
NES_ROM_PATH = (
    "/storage/emulated/0/TheVaultData/roms/NES/"
...
class VaultApp(App):
    ...
    def launch_retroarch_game(self, game: GameInfo):
        ...
        try:
            ...
            retroarch_apk = str(app_info.sourceDir)

            current_ime = SettingsSecure.getString(
                activity.getContentResolver(),
                SettingsSecure.DEFAULT_INPUT_METHOD,
            )
            ...
            data_dir = "/data/user/0/com.retroarch"
            ...
            intent.putExtra("ROM", game.path)
            intent.putExtra("LIBRETRO", game.core_path)
            intent.putExtra("CONFIGFILE", config_file)
            intent.putExtra("IME", current_ime or "")
            intent.putExtra("DATADIR", data_dir)
            intent.putExtra("APK", retroarch_apk)
            intent.putExtra("SDCARD", sdcard_dir)
            intent.putExtra("EXTERNAL", external_dir)
            ...
            print(
                "RetroArch launch:"
                f" package={RETROARCH_PACKAGE}"
                f" activity={RETROARCH_ACTIVITY}"
                f" core={game.core_path}"
                f" rom={game.path}"
